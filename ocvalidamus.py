    ...
    def load_config(self) -> dict:
        ...
        for p in FOLDERS_FILES_CHECK:
            if not (path / p).exists():
                raise FileNotFoundError(f"{Colors.FAIL}{p} not found{Colors.ENDC}")
        ...
        for kext in kexts:
            if kext['Enabled']:
                ...
                if path.exists():
                    ...
                    print(f"\t- {Colors.FAIL}{path}{Colors.ENDC} does not exist")
                    self.errors += 1

                for x in filter(None, (kext["ExecutablePath"], kext["PlistPath"])):
                    if (path / x).exists():
                        print(f"\t\t- {Colors.OKGREEN}{x}{Colors.ENDC} exists")
                    else:
                        print(f"\t\t- {Colors.FAIL}{x}{Colors.ENDC} does not exists")
                        self.errors += 1
            else:
